/***************************************
* INF2705 - TP1
* Samuel Charbonneau (1897953) et Jean-Olivier Dalphond (1873653)
* 12 septembre 2019
*****************************************/
...
public:
	...
	// attributs
        ...
        // on active l'attribut
        glEnableVertexAttribArray( locVertex );
	// On termine avec le vao
        glBindVertexArray(0);
    }
    ...
    void afficherTheiere()
    {
        ...
        // Partie 2 on dessine les connectivités
        glDrawElements( GL_TRIANGLES, sizeof(gTheiereConnec)/sizeof(GLuint), GL_UNSIGNED_INT, 0); // on utilise les 530 sommet du tableau de la théière
    ...
    void afficherCorps()
    {
		...
		matrModel.Rotate(angleCorps,0.0,0.0,1.0); // rotation du corps
            matrModel.Translate( 0.0, 0.0, -2*taille ); // pour bien positionner le corps
            matrModel.Scale( taille, taille, 2*taille); // pour bien scaler le corps
    ...
    void afficherTete()
    {
        ...
        matrModel.PushMatrix();{
            ...
            case 1: // une sphère
				...
				matrModel.Scale( taille, taille, taille); // pour bien scaler le corps
                glUniformMatrix4fv( locmatrModel, 1, GL_FALSE, matrModel );
                afficherSphere();
                break;

            case 2: // la théière
                matrModel.Scale( 0.45*taille, 0.45*taille, 0.45*taille); // pour avoir une taille de théière proportionnelle au corps
	...
	// nous avons créé un tableau pour gérer la symétrie des ailes et éviter la duplication du code
        int posParam[2] = {1,-1};
		for (int i = 0; i<=1;i++){
			// ajouter une ou des transformations afin de tracer des *ailes carrées*, de la même largeur que le corps
			matrModel.PushMatrix();{
				
				matrModel.Translate( 0.0, posParam[i]*taille, 0.0 ); // demi translation
				matrModel.Rotate(posParam[i]*angleAile,1.0,0.0,0.0); // rotation des ailes
				matrModel.Translate( 0.0, posParam[i]*taille, 0.0 ); // 2eme translations pour position finale
				
				matrModel.Scale( 2*taille, 2*taille, 1.0); // pour bien scaler le corps
    ...
    void afficherBrasJambes()
    {
        ...
        // les bras
	// nous avons créé un tableau pour gérer la symétrie des bras et des jambes et éviter la duplication du code
        int posParam[2] = {1,-1};
        for (int i = 0; i<=1;i++){
			matrModel.PushMatrix();{
				
				matrModel.Translate( 0.0, (taille)*posParam[i], 0.0 );
				matrModel.Rotate(posParam[i]*-45,1.0,0.0,0.0);
				matrModel.Translate( 0.0, posParam[i]*longMembre/2, 0.0 );
        ...
        for (int i = 0; i<=1;i++){
			matrModel.PushMatrix();{
					...
					matrModel.Translate( 0.0, posParam[i]*(taille), -1*(2*taille));
					matrModel.Rotate(angleJambe,0.0,1.0,0.0); // rotation des ailes
					matrModel.Translate( 0.0, 0.0, -1*(longMembre/2));
