...
public:
	Aquarium()
		: locplanRayonsX(-1), locattenuation(-1), enSelection(-1)
	{
		...
		for (unsigned int i = 0; i < sizeof(hauteur) / sizeof(hauteur[0]); ++i)
		{
			...
			int nbMaxDePoisson = sizeof(hauteur) / sizeof(hauteur[0]);
			int intervalle = 255/nbMaxDePoisson;
			Poisson *p = new Poisson(dist, hauteur[i], angle, vit, taille, double(i*intervalle)/255.0, double(i)/255.0, double(i)/255.0);
	...
	void initialiserGraphique()
	{
		...
		if ((enSelection = glGetUniformLocation(prog, "enSelection")) == -1) std::cerr << "!!! pas trouvé la \"Location\" de enSelection" << std::endl;
	}
	...
	void afficherContenu(GLenum ordre = GL_CCW)
	{
		...
		glFrontFace(ordre);
			...
			glUniform4f(locplanRayonsX, Etat::planRayonsX.x, Etat::planRayonsX.y, Etat::planRayonsX.z, Etat::planRayonsX.w);
	...
	void afficher()
	{
		...
		glUniform1i(enSelection, Etat::enSelection);
	...
	{
		//std::cout<<"selectionnerPoisson " << std::endl;
		//appeleé lorsqu'on clique
		glFinish();
		...
		int TauxRougePixel = couleur[0]; // taux de rouge du pixel
		for (std::vector<Poisson*>::iterator it = poissons.begin(); it != poissons.end(); it++)
		{
			if(TauxRougePixel <=(*it)->getTauxRouge()){
